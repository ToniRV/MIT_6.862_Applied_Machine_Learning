...
def get_data_loader(name, batch_size, validation_split=None):
    try:
        ...
        assert validation_split is None, 'Please make sure you are running the code from folder `code_for_hw7`'
        return None, None
    #print(loaded_data)
    ...
    if validation_split is not None:
        ...
        train_loader = DataLoader(dataset, batch_size=batch_size, collate_fn=SimpleCustomBatch)
        print('Loading train X', inps.shape, 'y', tgts.shape, 'classes', classes)

        val_index = index[-val_size:]
        inps, tgts = X[val_index], y[val_index]
        dataset = TensorDataset(inps, tgts)
        val_loader = DataLoader(dataset, batch_size=batch_size, collate_fn=SimpleCustomBatch)
...
def model_fit(model, train_iter, epochs, optimizer, criterion,
              validation_split, validation_iter, history, verbose):

  av_train_loss, av_train_acc, av_vali_loss, av_vali_acc = [], [], [], []
  for epoch in range(epochs):
      train_loss, train_acc_score = call_model('train', model, train_iter, optimizer, criterion)
...
def run_pytorch(train_iter, val_iter, test_iter, layers, epochs, split=0, 
    ...
    train_m, vali_m = model_fit(model, train_iter, epochs=epochs, 
                                ...
                                validation_split=split, validation_iter=val_iter,
